...
/// Simple view (#2463): an opt-in presentation of Today that keeps only the three scores, the one-line
/// synthesis and today's workouts, and makes each score open the wearer's own data behind it.
///
...
enum SimpleViewPrefs {
    ...
    static let keptSections: Set<TodaySection> = [.hero, .synthesis, .workouts]
    ...
    static func order(fullOrder: [TodaySection]) -> [TodaySection] {
        ...
        for section in [TodaySection.hero, .synthesis, .workouts] where !out.contains(section) {
            out.append(section)
        }
        return out
    }
